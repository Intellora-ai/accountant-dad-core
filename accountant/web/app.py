...
from accountant.tallyio.period import PeriodReader, reader_for
...
class Runtime:
    ...
    #: How this runtime asks Tally whether the books are open for a date.
    #:
    #: `None` MEANS NOBODY CAN LOOK, and it is the honest state for a runtime
    #: built by `configure()` with a double: there is no gateway behind a fake
    #: client, so a probe would be a socket call to whatever happens to be on
    #: port 9000 of the machine running the tests. That is how a suite becomes
    #: green on one laptop and red on another, and it is the reason this is a
    #: field rather than a default built inside `is_period_open`.
    #:
    #: `connect()` - the real path, and the only one that has a `TallyConfig` -
    #: builds one. So the shipped app measures and a doubled runtime says
    #: `None`, which BLOCKS. Neither of them guesses.
    period_reader: PeriodReader | None = None

    def period_open(self, on: datetime.date | None = None) -> bool | None:
        """Are this company's books open for `on`? Three answers, two of which block.

        `check_period`, NOT `is_period_open`, and the difference is a defect this
        code had until it was measured against what the customer would read.

        `is_period_open` returns a bare `bool`, so an unreachable Tally comes
        back as `False`. Handed to the cage, `False` means "the books are
        closed", and `decision._period_closed` renders *"The books for 12 March
        2026 are closed, so nothing can be added to them"* - a specific,
        confident statement about somebody's Tally, generated by OUR dropped
        connection. It sends them into their accounting software to fix a
        setting that was never wrong.

        So the three verdicts are kept apart all the way to the screen:

            OPEN        -> True    posts
            CLOSED      -> False   blocks, and says the books are closed
            UNVERIFIED  -> None    blocks, and says WE could not check

        `None` is the cage's own word for "nobody looked" and it already has its
        own sentence (`decision._PERIOD_UNKNOWN`). Both block - the owner's
        exchange rate is 100 false blocks to 1 silent wrong post - and the
        distinction buys the log and the sentence, never the decision.

        A runtime with no `period_reader` also answers `None`, which is the same
        fact arriving a different way: nobody looked.

        `on` is THE DATE ON THE BILL, passed by both call sites. Whether books
        are open is a question about a date, and a bill entered today for a
        purchase made in March is exactly the case this check exists for.
        """
        if self.period_reader is None:
            return None
        return check_period(
            company=self.company, on=on, reader=self.period_reader
        ).for_cage
...
def configure(
    ...
    *"This is your real Tally"* while every row in their own audit trail said
    ...
    built = Runtime(
        ...
        # Defaults to None, which is "this runtime cannot look up the period".
        # NOT a reader built here: `configure` has no `TallyConfig` - it is
        # handed a client that may be a double - so anything built here would
        # aim at localhost:9000 and answer about whatever is listening there.
...
def connect(
    ...
    *,
    ...
    return configure(
        ...
        # THE ONE PLACE A PERIOD READER IS BUILT, and it is here because this is
        # the one place that holds a `TallyConfig`. Same host and port as every
        # other read, and `reader_for` overrides only the patience: five seconds
        # and one attempt, so a gateway that hangs - which A11 records this one
        # doing on 2026-08-09 - becomes a caught error in front of a person
        # rather than a wedged request.
        #
        # `reader_for`, not `PeriodReader(HttpTransport(...))`. The transport is
        # what opens the socket and `tests/test_write_door.py` fails if anything
        # outside the connector imports it.
        period_reader=reader_for(config),
    )
...
def _run(data: bytes, mime: str) -> pipeline.Draft:
    ...
    d = pipeline.evaluate(
        ...
        live.memory,
        flag_cap=FLAG_CAP,
        # MEASURED FROM THE COMPANY'S OWN TALLY, 2026-08-13. This was `None` -
        # "nobody looked" - because nothing in this repository could look.
        # `accountant/tallyio/period.py` now reads the company's BOOKSFROM and
        # STARTINGFROM over the gateway and `accountant/period.py` turns every
        # failure into False, so the fact is read rather than asserted and an
        # unreadable answer still blocks.
        #
        # `on=d.voucher.date` AND NOT TODAY. Whether the books are open is a
        # question about the date on the BILL: an invoice typed in August for a
        # purchase made in March is the case this check exists for, and asking
        # about today would clear it while Tally would refuse it at the write
        # door. The draft exists two lines above, so its date is available here
        # and there is no reason to ask a weaker question.
        period_open=live.period_open(on=d.voucher.date),
        # This handler repaired nothing - it does not open the bytes at all. The
        # reader is what mends a broken PDF, and `cage/gate._repaired` takes the
        # answer off the record when it says so, overruling this `None`.
        pdf_repaired=None,
    )
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if self.path == "/answer":
            ...
            # The same two answers as the upload path, for the same reasons -
            # see the call in `_run`.
            #
            # RE-READ RATHER THAN REMEMBERED FROM THE FIRST REQUEST. An entry is
            # several HTTP requests and a person can sit on the review screen for
            # a long time; a period that was open when the bill was typed can be
            # closed by the time the question is answered, and this is the last
            # decision before the write. It is one bounded round trip.
            d = pipeline.evaluate(
                d,
                accounts,
                history,
