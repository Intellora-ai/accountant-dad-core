...
def test_posted_voucher_is_readable_back_from_tally():
    ...
    d = pipeline.run(
        COMPANY, typed("paid Sharma Traders 4200 cement"), "text/plain",
        TypedTextExtractor(), t, today=TODAY,
    )
...
def test_unknown_vendor_is_unclear_and_does_not_post():
    t = tally(past("Sharma Traders", "Purchases", n=5))
    d = pipeline.run(
        COMPANY, typed("paid Verma Cement 900 for bags"), "text/plain",
        TypedTextExtractor(), t, today=TODAY,
    )
...
def test_conflicted_vendor_asks_and_offers_only_accounts_seen_before():
    ...
    t = tally(hist)
    d = pipeline.run(
        COMPANY, typed("paid Verma Cement 900 bags"), "text/plain",
        TypedTextExtractor(), t, today=TODAY,
    )
    assert d.decision.outcome is Outcome.UNCLEAR
...
def test_posting_a_not_valid_draft_is_refused():
    ...
    index = MemoryIndex()
    d = pipeline.build_draft(
        COMPANY, typed("paid Nobody 0 for nothing"), "text/plain",
        TypedTextExtractor(), accounts, index, today=TODAY,
    )
...
def test_posting_an_unevaluated_draft_is_refused():
    ...
    d = pipeline.build_draft(
        COMPANY, typed("paid Sharma Traders 4200 cement"), "text/plain",
        TypedTextExtractor(), t.read_accounts(COMPANY), index, today=TODAY,
    )
    with pytest.raises(ValueError):
        pipeline.post(d, t)
...
def test_backend_outage_asks_the_person_to_type_instead():
    ...
    t = tally(past("Sharma Traders", "Purchases", n=5))
    d = pipeline.run(
        COMPANY, b"<pdf>", "application/pdf",
        UnavailableExtractor("provider timed out"), t,
    )
    assert d.decision.outcome is not Outcome.VALID
    assert d.posted_tally_id is None
...
def test_swapping_the_backend_changes_no_pipeline_code():
    ...
    a = pipeline.run(
        COMPANY, typed("paid Sharma Traders 4200 cement"), "text/plain",
        TypedTextExtractor(), t1, today=TODAY,
    )
    b = pipeline.run(
        COMPANY, b"x", "application/pdf",
        StubExtractor(date=TODAY, party="Sharma Traders", total_paise=420000, tax_paise=None),
        t2,
    )
...
def test_spelling_variants_of_one_vendor_are_the_same_vendor():
    ...
    for spelling in ("M/s Sharma Traders Pvt Ltd", "SHARMA TRADERS", "Sharma  Traders"):
        d = pipeline.run(
            COMPANY, typed(f"paid {spelling} 4200 cement"), "text/plain",
            TypedTextExtractor(), t, today=TODAY,
        )
...
def test_every_draft_carries_a_check_count():
    ...
    assert len(d.checks) == len(__import__("accountant.checks", fromlist=["x"]).ALL_CHECKS)
